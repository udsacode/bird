...
add_action( 'woo_loop_before', 'woo_loop_wrap_open' );
add_action( 'woo_loop_after', 'woo_loop_wrap_close' );

function woo_loop_wrap_open() {
	echo '<div class="content-container-fullwidth-nopadding blog-posts-grid">';
}

function woo_loop_wrap_close() {
	echo '</div>';
}

/*-----------------------------------------------------------------------------------*/
/* Add has-image class to posts */
/*-----------------------------------------------------------------------------------*/

add_filter( 'post_class', 'upstart_featured_image_class' );
function upstart_featured_image_class( $classes ) {
	global $post;
	$has_featured_image = get_the_post_thumbnail( $post->ID );
...
/*-----------------------------------------------------------------------------------*/
/* Fix image captions (removes additional 10px width) */
/*-----------------------------------------------------------------------------------*/
