...
/* Media settings */
$media_settings = array( 'width' => '2560', 'height' => '1440' );

if ( 'true' != $settings['featured_videotitle'] ) {
	$media_settings['width'] = '2560';
	$media_settings['height'] = '1440';
}
...
?>
<div id="featured-slider" class="flexslider <?php echo esc_attr( $container_css_class ); ?> content-container-fullwidth-nopadding">
	<ul class="slides">
<?php
	foreach ( $slides as $k => $post ) {
		...
		if ( '' != $embed ) {
			...
			$image = woo_image( 'width=2560&noheight=true&class=slide-image&link=img&return=true' );
			if ( '' != $image ) {
